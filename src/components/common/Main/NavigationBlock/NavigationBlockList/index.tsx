        ...
        <Box className={classes.main_container}>
            {mainServicesList.map((value, idx) =>
                <NavLink
                    ...
                    label={<Text fw={500} c={navigationStore.active === idx ? "white" : "#8B8B8B"}>{value.label}</Text>}
                    leftSection={<Image
                        className={navigationStore.active === idx ? classes.nav_link_icon_active : classes.nav_link_icon}
                        h={36} w="auto" fit="contain" src={value.icon}/>}
                    active={navigationStore.active === idx}
                    color="#5A84F1"
                    variant="filled"
                    p={6}
                    mb={6}
